...
def create_refresh_token(data: dict, expires_delta: timedelta = None):
    ...
    if expires_delta:
        ...
        expire = datetime.now(timezone.utc) + timedelta(minutes=refresh_token_expiry)  # Use timezone-aware datetime
...
def check_role(required: str):
    async def role_dependency(user: dict = Depends(get_current_user)):
        if user["role"] != required:
            raise HTTPException(status_code=403, detail="Forbidden: You don't have the right role")
        return user
